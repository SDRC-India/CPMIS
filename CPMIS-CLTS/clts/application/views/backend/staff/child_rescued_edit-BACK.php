          ...
          <div class="col-md-6">
            <div class="form-group">
              ...
              <div class="col-sm-8" <?php if($row['state']!='IND010'){ ?> style="display:none;" <?php } ?> id='inside' >
				...
				<span class="block-err-msg-edit  color-red"></span>
              </div>
           ...
           <div class="col-sm-6">
            ...
            <div class="form-group" >
              ...
              <div  id="pin_code_fr_grp"  class="col-sm-8 ">
               ...
               <div class="col-sm-8 pincode_form_grp" <?php if($row['state']=='IND010'){ ?> style="display:none;" <?php } ?> id="outside_pincode">
                <input type="text" class="form-control" maxlength="6"  onkeypress="return isNumberKey(event)"  name="pincode" id="pincode_form_grp"   value="<?php echo $row['pincode'] ; ?>">
            ...
            <div class="form-group top-margin">
             ...
             <div class="col-sm-8">
				...
				?>
				 ...
				 <option value="<?php echo $workinvoice['area_id'];?>"   ><?php echo $workinvoice['sector']; ?></option>
				 <?php endforeach; }else{ ?>
				 				 <option value="Other"  >Other</option>

				 <?php  foreach($workinvoice_list as $workinvoice): ?>
					 <option value="<?php echo $workinvoice['area_id'];?>"  <?php if($row2['work_involved']==$workinvoice['id']){ echo 'selected'; }  ?> ><?php echo $workinvoice['sector']; ?></option>
				<?php
				endforeach;
				}
...
var copmareAge_year = function(dor,dob) {
    ...
    function validate_project_add(formData, jqForm, options) {
		...
		if(jqForm[0].hidename_child.value==2){			
			 flag=1;
			 $(window).scrollTop(0);
            return false;
		}
...
//$(".contact2").focus();
$(".block-err-msg-edit").html("This field should not be blank!!");
return false;
}
else{
	
	  $("#block").removeClass("validate-has-error");
	 $(".block-err-msg-edit").html("");
...
if(document.getElementById("pincode_form_grp").value)
{
	
	if(document.getElementById("pincode_form_grp").value.length!=6)
	{
		//alert(document.getElementById("pincode_form_grp").value.length);
	
  flag=1;
 
  $("#pincode_form_grp").addClass("validate-has-error");
  //$(".contact2").focus();
  $(".pincode-err-msg").html("This field should be 6 numeric digits");
 return false;
	}
	else{
		
		  $("#pincode_form_grp").removeClass("validate-has-error");
		 $(".pincode-err-msg").html("");
