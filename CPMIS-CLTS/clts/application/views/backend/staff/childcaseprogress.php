

<!------------------start of family list-------------------------->

	
		<form class="form-inline"  id="myform" name="" method="post" action="<?php echo base_url(); ?>index.php?childcaseprogress/record">
	<?php 
	
	 $past_date=date('Y-m-d', strtotime(date('Y-m')." -1 month"));
	 $currentDate = date("Y-m-d");
	
	?>
	 
	
		<div class="col-md-4 report" >
		<div class="col-md-2">
           	<label for="datepicker" style="margin-top:6px;" >From</label>
           	 </div>
           	 <div class="col-md-10">
                <div class="input-group date" id="datepicker"> <span class="input-group-addon"><i class="glyphicon glyphicon-calendar"></i></span>
                  <input type="text" class="form-control " name="from_date" id="from_date" required="required" value="<?php  if($from==""){echo $past_date; }else {echo $from; } ?>" data-validate="required"  >
                	</div>
            </div>
            </div>
		   <div class="col-md-4">
        		<div class="col-md-2">
           	<label for="datepicker" style="margin-top:6px;">To</label>
           	 </div>
           	 <div class="col-md-10">
                <div class="input-group date" id="datepickerTo"> <span class="input-group-addon"><i class="glyphicon glyphicon-calendar"></i></span>
                  <input type="text" class="form-control " name="to_date" id="to_date" required="required"  value="<?php  if($to==""){echo $currentDate;}else {echo $to; } ?> "  data-validate="required"  >
                </div>
                 </div>
		  </div>
		   
  			<div class="col-md-2">
        
  			<button type="button" class="btn btn-info submit" id="submit">Submit</button>
  			</div>


	</form>



<br/>
<br/>
<br/>
<script>
$(document).ready(function () {
$("#submit").on("click",function(){
			 			 
			 $('#loading').show();
		 });
});
 </script>
 
<div class="row">
<div class="modal fade" id="loaderId" role="dialog">
    <div class="modal-dialog">
      <div class="loader"></div>
    </div>
 </div>

<table class="table table-bordered datatable" id="table_export">
  <thead>
    <tr>
	  <th>Sl No</th>
      <th><div>Child ID</div></th>
	  <th style= "width:30px"><div>Child Name</div></th>
	 
      <th><div>Percentage Of Progress</div></th>
       </tr>
  </thead>
  <tbody>
  
    <?php
         $i=1;
    	$k = 0;
		foreach($percentage as $row):
			$complete = round(100 - $row['percentage']);
		?>
    <tr>
      <td><?php echo $i;?></td>
      <td><a href="<?php echo base_url().'index.php?/child_rescued_list/view/'.$row['id'];?>"><?php echo $row['id'];?></a> </td>
	  <td  style= "width:30px"><?php echo $row['child_name'];?></td>
	 
      </td>
      <td>
	  <div class="progress">
  <a href="<?php echo base_url().'index.php?/child_rescued_list/view/'.$row['id'];?>"><div class="progress-bar progress-bar-info" role="progressbar" aria-valuenow="50"
  aria-valuemin="0" aria-valuemax="100" style="width:<?=$complete?>%">
    <?=$complete?>% 
  </div></a>
</div>
      </td>
    </tr>
    <?php  $i++;
		endforeach;?>
  </tbody>
</table>
</div>
<!-- ----------------end of family list---------------------------->
<script type="text/javascript">

	jQuery(document).ready(function($)
	{
		var today = new Date();
			var dd = today.getDate();
			var mm = today.getMonth()+1; //January is 0!
			var yyyy = today.getFullYear();

			if(dd<10) {
				dd='0'+dd
			} 

			if(mm<10) {
				mm='0'+mm
			} 

			today = dd+'-'+mm+'-'+yyyy;
			var title="Child Case progress";
             var from=$("#to_date").val();
	  var to=$("#from_date").val();
     title=title+'\n'+'FROM-'+from+' '+'TO-'+to;
		// convert the datatable
		var datatable = $("#table_export").dataTable({
			"sPaginationType": "bootstrap",
			"paging": true,
			"dom": 'Blftrip',
			"columnDefs": [
          { "orderable": false, "targets": 3}
        ],
      "aoColumns": [
        
				{ "bSortable": true },
				{ "bVisible": true},
				{ "bVisible": true},
				{ "bVisible": true},
				
			],
			
			buttons: [ {extend: 'excelHtml5',
				autoPrint: true,
				header: true,
				text:'<i class="fa fa-file-excel-o"></i> Excel',
				title: "MIS Report-"+title
			},{
				extend: 'print',
				autoPrint: true,
				header: true,
				text:'<i class="fa fa-print"></i> Print',
				title: "MIS Report-"+title
			},{
            extend: 'pdfHtml5',
            text: '<i class="fa fa-file-pdf-o"></i> PDF ',
			title:title,
           customize: function ( doc ) {
			   
			   doc.styles.tableBodyEven={
		      
				
			    alignment: 'center'
			   };
				doc.styles.tableBodyOdd={
					
					alignment: 'center'
				};
			   var cols = [];
			   cols[0] = {image: 'data:image/png;base64,', alignment: 'left', margin:[10,0,20] };
			   
			   cols[1] = {text: 'CLTS', alignment: 'right', margin:[10,10,20] };
			   var objHeader = {};
			   objHeader['columns'] = cols;
			   doc['header']=objHeader;
			   var cols = [];
			   cols[0] = {text: 'Supported By Unicef', alignment: 'left', margin:[20] };
			   cols[1] = {text: 'Printed ON:'+today+'from http://www.cpmis.org/clts ', alignment: 'center',};
			   cols[2] = {text: 'Powered By SDRC', alignment: 'right', margin:[0,0,20] };
			   var objFooter = {};
			   objFooter['columns'] = cols;
			   doc['footer']=objFooter;
               
               
                // Data URL generated by http://dataurl.net/#dataurlmaker
            }
        } ],
		
	
	
		});
	

	});
	
	
	
</script>
<script>
$(".submit").on("click",function(){
var from=$("#from_date").val();
var to=$("#to_date").val();
window.location.assign('<?php echo base_url(); ?>index.php?childcaseprogress/record/'+from+'/'+to);

});

</script>
<script>
	var FromEndDate = new Date();
$('#datepicker').datepicker({
format: 'yyyy-mm-dd',
endDate: FromEndDate,
autoclose: true,
"setDate": new Date()
});
</script>
	<script>
var FromEndDate1 = new Date();
$('#datepickerTo').datepicker({
format: 'yyyy-mm-dd',
endDate: FromEndDate1,
autoclose: true,
date:FromEndDate1
});
</script>
<script src="assets/js/neon-custom-ajax.js"></script>
