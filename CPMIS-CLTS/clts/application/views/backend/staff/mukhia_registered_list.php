...
<table class="table table-bordered datatable" id="table_export">
	<thead>
		<tr>
			...
			<th><div><?php echo get_phrase('District Name');?></div></th>
			<th><div><?php echo get_phrase('Block Name');?></div></th>
			<th><div><?php echo get_phrase('Panchayat Name');?></div></th>
			<th><div><?php echo get_phrase('Address of residence');?></div></th>
		</tr>
	</thead>
	<tbody>
	<?php
	$counter=1;
		...
		$result=$row['dist_areaid'] ;
		$panchayat_id=$row['panchayat_id'] ;
		$res_distname=mysql_fetch_assoc(mysql_query("select area_name from child_area_detail where area_id='$result'")) ;
		$res_panchayat_name=mysql_fetch_assoc(mysql_query("select id,name,block_id from panchayat_names where id='$panchayat_id'")) ;
		$block_id=$res_panchayat_name['block_id'];
	$res_blockname=mysql_fetch_assoc(mysql_query("select area_name from child_area_detail where area_id='$block_id'")) ;
      ...
      <td><?php echo $res_distname['area_name'] ;  ?></td>
      <td><?php echo $res_blockname['area_name'] ;  ?></td>
      <td><?php echo $res_panchayat_name['name'] ;  ?></td>
...
<script type="text/javascript">
	...
	jQuery(document).ready(function($)
	{
		var today = new Date();
			...
			today = dd+'-'+mm+'-'+yyyy;
			var title=" List of Registered Mukhia";
