<!-- ------------------------start of economy list----------------- -->
<?php  //echo $typee;?>

<input type="hidden" id="typee" value="<?php  echo $typee;?>">

<div style = "float:right">
<?php
if ($roles==10 || $roles==13 || $roles==20 || $roles==12) {?>

<a href ="index.php?comunication/comm_view" ><button  class ="btn btn-info" /><i class="entypo-chat" aria-hidden="true"></i> Start Communication</button></a>

<?php }?>
<?php
if($roles==21) {?>

<a><button  class ="btn btn-info" /><i class="entypo-chat" aria-hidden="true"></i> Start Communication</button></a>

<?php } ?>

</div>
<table class="table table-bordered datatable" id="table_export">
  <thead>
    <tr>
      <th class="table_td_width"> Sl No</th>
	  <?php if($roles==10 || $roles==21 || $roles==12){?>
      <th><div>Designation</div></th>
	  <th><div>District</div></th>
	  <?php }  ?>
      <th><div>Message</div></th>
	 <th><div>child Id</div></th>
      <th style="width:100px;"><div>Date Of communication</div></th>
      <?php 
      if ($roles!=5 ) {?>
      <th><div>Reply Message</div></th>
	  <?php }?>
     <?php
     if ($roles==10 || $roles==21 || $roles==12 ) {?>
      <th class="table_td_width1 view-button"><div><?php echo get_phrase('options');?></div></th>
	 <?php }?>
	  <?php
     if ($roles==2 ) {?>
      <th class="table_td_width1 view-button" ><div><?php echo get_phrase('Reply');?></div></th>
	 <?php }?>
    </tr>
  </thead>
  <tbody>
    <?php 
    $counter = 1;
   //print_r($communication);
    foreach($communication as $row):

			?>
    <tr>
      <td class="table_td_width"><?php echo $counter++;?> </td>
	  <?php if($roles==10 || $roles==21 || $roles==12){?>
      <td><?php echo $row['role_name'] ?></td>
	  <td><?php echo $row['area_name'] ?></td>
	  <?php }  ?>
      <td><?php echo $row['massage'] ?></td>
	  <td><a href="<?php echo base_url()?>index.php?child_rescued_list/view/<?php echo $row['child_id'];?>"><?php echo $row['child_id'] ?></a></td>
      <td><?php echo $row['created'] ?></td>
      <?php 
      if ($roles!=5 ) {?>
        <td><?php echo $row['reply_message']?></td>
        <?php }?>
	    <?php
	    
	    if($row['reply_message']=="")
	    {
	    
	    if ($roles==10 || $roles==12) {?>
      <td class="view-button" style="text-align:center;">
	 
        <a class="btn btn-info tooltip-primary btn-edit" data-toggle="tooltip" data-placement="top" title="" data-original-title="View"	href="index.php?comunication/comm_viewdetails/<?php echo $row['id'];?>" > <i class="entypo-eye"></i> </a>
	 
      </td>
	   <?php }?>
	     <?php
	    if ($roles==2) {?>
      <td class="view-button" style="text-align:center;" >	 
        <a class="btn btn-info tooltip-primary btn-edit" data-toggle="tooltip" data-placement="top" title="" data-original-title="Reply"	href="index.php?comunication/comm_reply/<?php echo $row['id'];?>" > <i class="fa fa-reply-all"></i> </a>	 
      </td>
	   <?php }?>
	    <?php
	    if ($roles==21) {?>
      <td class="view-button" style="text-align:center;">
	 
         <a class="btn btn-info tooltip-primary btn-edit" data-toggle="tooltip" data-placement="top" title="" data-original-title="Edit" ><i class="entypo-pencil"></i></a>
	 
      </td>
	   <?php }?>
	   
	 <?php }else{?>
	 
	<?php   if ($roles==10 || $roles==12) {?>
	 <td class="view-button" style="text-align:center;">
	    <a class="btn btn-info tooltip-primary btn-edit" data-toggle="tooltip" data-placement="top" title="" data-original-title="View" href="index.php?comunication/comm_reply/<?php echo $row['id'];?>" ><i class="entypo-eye"></i></a>
	     </td>
	  <?php }?>
	 
	  
	<?php   if ($roles==2) {?>
	 <td class="view-button" style="text-align:center;">
	    <a class="btn btn-info tooltip-primary btn-edit" data-toggle="tooltip" data-placement="top" title="" data-original-title="View" href="index.php?comunication/comm_reply/<?php echo $row['id'];?>" ><i class="entypo-eye"></i></a>
	     </td>
	  <?php }?>
	 
	 
	 <?php   if ($roles==21) {?>
	 <td class="view-button" style="text-align:center;">
	    <a class="btn btn-info tooltip-primary btn-edit" data-toggle="tooltip" data-placement="top" title="" data-original-title="View" href="index.php?comunication/comm_reply/<?php echo $row['id'];?>" ><i class="entypo-eye"></i></a>
	     </td>
	  <?php }?>
	 
	 
	 <?php }?>
    </tr>
    <?php

		endforeach;?>
  </tbody>
</table>
<!------------------end of economy list------------------>
<script type="text/javascript">
jQuery(document).ready(function($)
	{
	
	var today = new Date();
			var dd = today.getDate();
			var mm = today.getMonth()+1; //January is 0!
			var yyyy = today.getFullYear();

			if(dd<10) {
				dd='0'+dd
			} 

			if(mm<10) {
				mm='0'+mm
			} 

			today = dd+'-'+mm+'-'+yyyy;
			var title="Communication";
     
		// convert the datatable
	/*	var datatable = $("#table_export").dataTable({
			"sPaginationType": "bootstrap",
			
			"dom": 'Bflit',
			buttons: [ {extend: 'excelHtml5',
				autoPrint: true,
				header: true,
				text:'<i class="fa fa-file-excel-o"></i> Excel',
				title: "<center>"+title+"</center>",
				exportOptions: {
                    		columns: [ 0, 1, 2, 3, 4, 5]         

                },
			},{
				extend: 'print',
				autoPrint: true,
				header: true,
				text:'<i class="fa fa-print"></i> Print',
				title: "<center>"+title+"</center>",
				exportOptions: {
					columns: [ 0, 1, 2, 3, 4, 5]         
                }
            
			},{
            extend: 'pdfHtml5',
            text: '<i class="fa fa-file-pdf-o"></i> PDF ',
			title:title,
			exportOptions: {
					columns: [ 0, 1, 2, 3, 4, 5]         
					},
           customize: function ( doc ) {
			   
			   	styles: {alignment: 'center'}
			   var cols = [];
			   cols[0] = {image: 'data:image/png;base64,', alignment: 'left', margin:[10,0,20] };
			   
			   cols[1] = {text: '', alignment: 'right', margin:[10,10,20] };
			   var objHeader = {};
			   objHeader['columns'] = cols;
			   doc['header']=objHeader;
			   var cols = [];
			   cols[0] = {text: 'Supported By Unicef', alignment: 'left', margin:[20] };
			   cols[1] = {text: 'Printed ON:'+today+'from http://www.cpmis.org/clts ', alignment: 'center',};
			   cols[2] = {text: 'Powered By SDRC', alignment: 'right', margin:[0,0,20] };
			   var objFooter = {};
			   objFooter['columns'] = cols;
			   doc['footer']=objFooter;
               
               
                // Data URL generated by http://dataurl.net/#dataurlmaker
            }
        } 
			],
		"dom": 'Bflit'
	
	
		});*/
			// convert the datatable
			var datatable = $("#table_export").dataTable({
				"sPaginationType": "bootstrap",
				//code add by poojashree
				//for displaying all data in list table
				 "lengthMenu": [[25, 50, 100, -1], [25, 50, 100,"All"]],
				"paging": true,
				"dom": 'Blftrip',
				buttons: [ {
					
					text:'<i class="fa fa-file-excel-o button-excel"  ></i> Excel',
					title: "Communication"
				},{
					extend: 'print',
					autoPrint: true,
					header: true,
					text:'<i class="fa fa-print"></i> Print',
					title: "MIS Report-"+title
				},{
	            extend: '',
	            text: '<a href="#" data-type="pdf" class="mis_submit"><i class="fa fa-file-pdf-o"></i> PDF </a>',
				
	        } ],
			
		
		
			});

			$(".mis_submit").on("click",function(){
				//alert("hii");
			       var type1="communication"; 
			       var typee=$('#typee').val();
			       //alert(typee);
			     var export_type=$(this).attr("data-type");
			      //alert(export_type);
			      // var from=$("#from_dt").val();
			      // alert(from);
			      // var to=$("#to_dt").val();
			       //alert(to);
			      // var dist_id=$("#dist_id").val();
			       //alert(dist_id);

           
			

			if(typee=="LC"){

			 window.location.assign('<?php echo base_url(); ?>index.php?comunication/LC/'+export_type);
			}
			
			else if(typee=="JLC"){
			 window.location.assign('<?php echo base_url(); ?>index.php?comunication/JLCmesg/'+export_type);

			}

			else if(typee=="ALC"){
			 window.location.assign('<?php echo base_url(); ?>index.php?comunication/ALCmesg/'+export_type);

			}

			
			else if(typee=="DLC"){
			 window.location.assign('<?php echo base_url(); ?>index.php?comunication/DLCmesg/'+export_type);

			}
			else{
             
		       window.location.assign('<?php echo base_url(); ?>index.php?comunication/index/'+export_type);

			}

			
			});

		

	});

$(document).ready(function(){
	$(".button-excel").closest('.dt-button').on("click",function(e){
		var type=$("#type").val();
		//var rehabilitation_section=$("#rehabilitation_section").val();
		//alert(rehabilitation_section);
		 //var to_date=$("#to_dt").val();
		//var frm_date=$("#from_dt").val();
		fnExcelReport("table_export",type);
	});

	function fnExcelReport(id,type)
	{
		type="Communication";
									
		//code ended for excel heading	
	   //code ended for excel heading	
	//code changed for excel sheet link removal
    var tab_text="<center><div style='color:#0054a5;font-size:20px;'><b>"+type+"</b></div><div style='color:#0054a5;font-size:15px;'></div></center><table border='2px'>";
    var textRange; var j=0;
    tab = document.getElementById(id); // id of table
     var tabSelector = $("#"+id);
    var columnCount = $("#"+id).find("thead tr:first-child th").length;
    for(var i=1; i<=$("#"+id).find("thead tr:first-child th").length; i++){
    	if(!$("#"+id).find("thead tr:first-child th:nth-child(" + i +")").hasClass("view-button")){
    	 tab_text = tab_text + tabSelector.find("thead tr:first-child th:nth-child(" + i +")").get(0).outerHTML;
    	}
        }
//     for(j = 1; j<)
// 	tabSelector.find("a").parent().html(tabSelector.find("a").html())
     
    for(j = 1 ; j <= $("#"+id).find("tbody tr").length ; j++) 
    {     
        tab_text = tab_text + "<tr>"
        for(k = 1; k <= columnCount; k++){
            if(!$("#"+id).find( "tbody tr:nth-child(" +j +")").find("td:nth-child(" + k + ")").hasClass("view-button")){
            	 if($("#"+id).find( "tbody tr:nth-child(" +j +")").find("td:nth-child(" + k + ")").find("a").length){
                 	tab_text = tab_text + '<td style="">' + $("#"+id).find( "tbody tr:nth-child(" +j +")").find("td:nth-child(" + k + ")").find("a").html() + "</td>";
                 }
                 else{
                 	tab_text=tab_text+$("#"+id).find( "tbody tr:nth-child(" +j +")").find("td:nth-child(" + k + ")").get(0).outerHTML;
                     }
                }
           
        	
        }
        
        tab_text=tab_text+"</tr>";
    }
   
    tab_text=tab_text+"</table>";
    tab_text= tab_text.replace(/<A[^>]*>|<\/A>/g, "");//remove if u want links in your table
    tab_text= tab_text.replace(/<img[^>]*>/gi,""); // remove if u want images in your table
    tab_text= tab_text.replace(/<input[^>]*>|<\/input>/gi, ""); // reomves input params
	console.log(tab_text);    
    var ua = window.navigator.userAgent;
   var msie = ua.indexOf("MSIE "); 

   if (msie > 0 || !!navigator.userAgent.match(/Trident.*rv\:11\./))      // If Internet Explorer
  {
       txtArea1.document.open("txt/html","replace");
       txtArea1.document.write(tab_text);
       txtArea1.document.close();
        txtArea1.focus(); 
        sa=txtArea1.document.execCommand("SaveAs",true,"gridTableData.xls");
   }  
   else                 //other browser not tested on IE 11
//         sa = window.open('data:application/vnd.ms-excel,' + encodeURIComponent(tab_text));
   //code add by poojashree 
   //for naming excel sheet for mis report
        var link = document.createElement("a");
link.download = type+".xls";
link.href = 'data:application/vnd.ms-excel,' + encodeURIComponent(tab_text);
link.click();
    
}

	
});

	
</script>
<script src="assets/js/neon-custom-ajax.js"></script>
