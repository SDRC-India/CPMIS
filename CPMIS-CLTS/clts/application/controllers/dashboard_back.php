...
class Dashboard extends MY_Controller
{
    ...
    // Age wise brackup    
        ...
        public function get_age() {
        	...
        	{
        		$monthly_data_report_details['agewise_brakeup']=$this->dashboard_model->get_agewise($from,$to,$dist);
        		//print_r($monthly_data_report_details['agewise_brakeup']) ;
        		$this->data['title']="Age-Wise Brack-Up";  
        		$shirt = array(
        				'color' => 'blue',
        				'number' => 23,
        				'size' => 'XL'
        		);
        		$monthly_data_report_details['shirt']=$shirt ;
        		$this->data['main_content_view'] = $this->load->view('backend/staff/age_wise_chart',$monthly_data_report_details, TRUE);
        		$this->generate_data('backend/index', $this->data );       		
        	}
        }
		...
		else if( $roles_id==13 || $roles_id==20 )//DLC
		{
		  $child_dist =$this->db->query("select child_area_detail.area_name as area_name,child_area_detail.area_id from child_area_detail
        	...
        	for($i=0; $i<sizeof($child_dist); $i++){
			 ...
			 $cumulative[sizeof($child_dist)][5]=$tot1+$tot2+$tot3+$tot4;
			// $cumulative[38][5]=$tot1+$tot2+$tot3+$tot4;
			 if($type=='view')
			 {
			 	...
			 	//print_r($cumulative);
			 	$data['lc_data']=$cumulative;
			 	 $this->load->view('backend/staff/pdf_lc_export.php',$data);
			 	//$this->data['main_content_view'] = $this->load->view('backend/staff/pdf_lc_export.php',$cumulative, TRUE);
