...
class New_cwc extends MY_Controller
{
          ...
          public function index($type)
          {
          	...
          	$middle_list['editmiddle_url']=$this->model_new_cwc->get_cwc_list_new($abc);
          	$middle_list["editmiddle_url"]=base_url()."index.php?new_cwc/cwc_member_edit/";
          	$url=base_url().'index.php?new_cwc/new_cwc_add/';
          	$middle_list['add_button']='<a href="'.$url.'"
                class="btn btn-info pull-right">
                    <i class="entypo-user-add"></i>
                  Add New CWC Member
                </a>';
          	$this->data['title']="List of Own CWC Member";
          	$this->data['default']="uploads/user.jpg";
          	$middle_list["type1"]="List_of_own_cwc_member";
          	//code added by pooja for middle man pdf download
          	//On 07.08.17
          	if($type=="pdf")
          	{
          		$name="List_of_own_cwc_member";
	 ...
	 public function create()		 
	 {
		if ($this->session->userdata('staff_login') != 1)
		 ...
		 public function cwc_member_edit($param1="")
		 {
		 	$this->load->model('model_new_cwc');
		 	$ses_ids=$this->session->userdata('login_user_id');		 	
		 	$this->data['title']="Edit CWC Member";
		 	...
		 	$this->data['main_content_view'] = $this->load->view('backend/staff/new_cwc_member_edit.php', $cwc_data, TRUE);
		 	$this->generate_data('backend/index', $this->data );
		 } 
		 ...
		 public function new_cwc_add($param1="")
		 {
		 	...
		 	$cwc_data['designation_list']=$this->model_new_cwc->get_designation_list($staff_id);
		 ...
		 {	
			...
			$ses_ids=$this->session->userdata('login_user_id');
			$cwc_data["editmiddle_url"]=base_url()."index.php?new_cwc/cwc_member_edit/";
