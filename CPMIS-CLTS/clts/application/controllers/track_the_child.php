...
class Track_the_child extends MY_Controller
{
          ...
          if($param1!='list'){
               ...
               $child_id = $this->input->post('child_id');
				...
				if(!$this->input->post('to_dt'))
					...
					if($role_id =='9' || $role_id=='10' || $role_id=='11' || $role_id=='21' || $role_id=='12' || $role_id=='22'){
					$child_dist = $this->input->post('dist_name');
				...
				$quer="Select * from child_basic_detail where TRUE "
					...
					AND district in(select district_id from division_details where division_id='".$district_id."') AND STR_TO_DATE(date_of_creation,'%d-%m-%Y') BETWEEN  '".$from_dt."' AND '".$to_dt."' ";
				}
				else{
					$child_block = $this->input->post('block_name');
					$quer="Select * from child_basic_detail where TRUE "
							...
							($child_dist!=null ?" AND district_id='".$child_dist."' ":"" ).
							($child_block!=null ?" AND block='".$child_block."' ":"" )."
					AND district_id='".$district_id."' or child_id in(select child_id from final_order where dist='" .$district_id . "') AND STR_TO_DATE(date_of_creation,'%d-%m-%Y') BETWEEN  '".$from_dt."' AND '".$to_dt."' ";
