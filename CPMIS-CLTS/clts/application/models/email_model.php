...
/*        if ($task == 'support_ticket_assign_staff')
		...
		if ($task == 'password_create_ngo')
        {
            $account_type   =   $param2;
            $email          =   $param3;
            $NEW_PASSWORD   =   "Your Password is: ".$param4;
            $NAME           =   $this->db->get_where('ngo_reg' , array('email' => $email))->row()->name;
            $SYSTEM_URL     =    "http://www.cpmis.org/login_ngo.php" ;

            //$email_msg      =   str_replace('[NAME]' , $NAME , $email_msg);
            $email_msg      =   'Dear '.$NAME.'<br>'."Your Password is: ".$param4.'<br>'.'Application url '.$SYSTEM_URL ;
            //$email_msg      =   str_replace('[SYSTEM_URL]' , $SYSTEM_URL , $email_msg);
            $email_to       =   $email;
        }
	...
	function do_email($msg=NULL, $sub=NULL, $to=NULL, $from=NULL, $attachment_url=NULL)
	{
		...
		$config['smtp_user']	= "unicef.vlms@gmail.com";//Need to change
		$config['smtp_pass']	= "vlms@123";//Need to change
        $config['mailtype']		= 'html';
        $config['charset']		= 'iso-8859-1';
        $config['newline']		= "\r\n";
        $config['wordwrap']		= TRUE;

        $this->load->library('Email');//Author Avinash
