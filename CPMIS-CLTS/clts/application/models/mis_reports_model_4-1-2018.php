	...
	if($param1=="pdf")
	  ...
	  //district wise rescued child list
       ...
       public function	get_details_rescuedchild_district($from,$to,$type,$area)
       {
		 ...
		 STR_TO_DATE(idate_of_rescue,'%Y-%m-%d') BETWEEN  '".$from."' AND '".$to."'
         AND child_area_detail.area_id='".$area."' AND
         staff.name='".$type."' AND child_basic_detail.district_id COLLATE utf8_unicode_ci in (select district_id from division_details  where division_details.division_id='" .$division_id."') ";
	  ...
	  //child rescued outside bihar data
       public function report_details_outside($from,$to,$state="",$dist)
	  {
	  	if($dist){
	  		...
	  		if($state){
		       ...
		       WHERE child_basic_detail.district='".$dist."' AND basic_place_of_rescue='Outside State' AND child_outside_state.outside_state='".$state."'  AND STR_TO_DATE(idate_of_rescue,'%Y-%m-%d') BETWEEN  '".$from."' AND '".$to."' GROUP BY child_area_detail.area_name ")->result_array();
		        ...
		        left join child_area_detail ON child_outside_state.outside_state=child_area_detail.area_id where child_outside_state.outside_state!='IND010'order by area_id")->result_array();
	  			...
	  			echo "select child_area_detail.area_name as area_name,child_area_detail.area_id as area_id from child_basic_detail
		        left join child_outside_state ON child_basic_detail.child_id=child_outside_state.child_id
		        ...
		        GROUP BY child_area_detail.area_name LIMIT 1, 9999999  ";
	  		...
	  		//echo $child_dist;
	  	     	...
	  	     	$child_outside_state=$this->db->query("select count(child_basic_detail.child_id) as count,
child_area_detail.area_id as area_id from child_basic_detail
left join child_outside_state ON child_basic_detail.child_id=child_outside_state.child_id
left join child_area_detail ON child_outside_state.outside_state=child_area_detail.area_id
left join child_area_detail as v on v.area_id=child_basic_detail.district
WHERE  basic_place_of_rescue='Outside State' AND child_outside_state.outside_state='IND028'
AND v.area_id='IND010028'
AND STR_TO_DATE(idate_of_rescue,'%Y-%m-%d') BETWEEN '".$from."' AND '".$to."' ")->result_array();
...
// 		           WHERE  basic_place_of_rescue='Outside State' AND child_outside_state.outside_district='".$area."' AND STR_TO_DATE(idate_of_rescue,'%Y-%m-%d') BETWEEN  '".$from."' AND '".$to."' ";
		    ...
		    $quer1="select child_basic_detail.child_id,child_name,
(select child_area_detail.area_name from child_area_detail where 
child_basic_detail.district=child_area_detail.area_id ) as child_dist,
father_name,postal_address as postal_address,idate_of_rescue,
child_area_detail.area_id as area_id from child_basic_detail 
left join child_outside_state ON child_basic_detail.child_id=child_outside_state.child_id 
left join child_area_detail ON child_outside_state.outside_state=child_area_detail.area_id 
left join child_area_detail as v on v.area_id=child_basic_detail.district
WHERE  basic_place_of_rescue='Outside State' AND child_outside_state.outside_state ='".$area."'
 AND v.area_id='".$dist_id."'
...
// 		              STR_TO_DATE(idate_of_rescue,'%Y-%m-%d') BETWEEN  '".$from."' AND '".$to."' GROUP BY child_area_detail.area_name ")->result_array();
			  	...
			  	$ls_list=$this->db->query("select y.area_id as area_id, 
SUM(CASE WHEN account_role.name = 'LS' THEN 1 ELSE 0 END) LS 
from child_area_detail as y join child_basic_detail as x ON y.area_id=x.district_id join 
staff ON x.uid=staff.staff_id join account_role ON 
staff.account_role_id=account_role.account_role_id left 
join final_order as z on x.child_id =z.child_id 
where x.child_id in(select z.child_id from final_order where y.parent_id ='IND010' and dist='' or dist IS NULL) 
AND STR_TO_DATE(idate_of_rescue,'%Y-%m-%d') BETWEEN  '".$from."' AND '".$to."' GROUP BY y.area_name
")->result_array();
			  ...
			  else if($roles_id==13 || $roles_id==20)
			  {
			  	...
			  	$ls_list=$this->db->query("select child_area_detail.area_id as area_id, SUM(CASE WHEN account_role.name = 'LS' THEN 1 ELSE 0 END) LS
			          ...
			          staff.account_role_id=account_role.account_role_id
			          join final_order  on child_basic_detail.child_id =final_order.child_id 
where child_basic_detail.child_id in(select final_order.child_id from final_order where child_area_detail.parent_id ='IND010' and dist='' or dist IS NULL) and
		              STR_TO_DATE(idate_of_rescue,'%Y-%m-%d') BETWEEN  '".$from."' AND '".$to."' GROUP BY child_area_detail.area_name ")->result_array();
			  	$cwc_list=$this->db->query("select child_area_detail.area_id as area_id, SUM(CASE WHEN  account_role.name = 'CWC' THEN 1 ELSE 0 END) CWC
                      from child_area_detail
                      join  child_basic_detail ON
	  ...
	  public function get_cumulative($from,$to,$type="") {
	    ...
	    $stat_id=$rolep['state_id'];
	  	...
	  	if( $roles_id==10 || $roles_id==11 || $roles_id==21 || $roles_id==12 || $roles_id==22)//For LC and LC Operator
	  	{
	  		...
	  		//echo "select  count(x.child_id) as num_child,y.area_id, y.area_name from child_area_detail as y left join child_basic_detail as x on x.district_id =y.area_id where y.parent_id ='IND010' AND  STR_TO_DATE(x.idate_of_rescue,'%Y-%m-%d') BETWEEN  '".$from."' AND '".$to."'  group by area_name order by y.area_name";
	  		
	  		//echo "select area_name,area_id from child_area_detail where parent_id='IND010' group by area_name order by area_name" ;
	  		$child_dist =$this->db->query("select area_name,area_id from child_area_detail where parent_id='IND010' group by area_name order by area_name")->result_array();
	  		
	  		$child_resc = $this->db->query("select count(x.child_id) as num_child,y.area_id, y.area_name from
child_area_detail as y 
left join  child_basic_detail as x on x.district_id =y.area_id 
left join  final_order as z on x.child_id =z.child_id 
where (x.district_id='' and z.child_id
in(select x.child_id from final_order where dist='' or dist IS NULL)) 
or x.child_id in(select z.child_id from final_order where y.parent_id ='IND010') 
AND STR_TO_DATE(x.idate_of_rescue,'%Y-%m-%d') 
BETWEEN  '".$from."' AND '".$to."'  group by area_name order by y.area_name")->result_array();
	  	...
	  	else if( $roles_id==13 || $roles_id==20 )//DLC
	  	{
	  		...
	  		$child_resc = $this->db->query("select count(x.child_id) as num_child,y.area_id, 
y.area_name from child_area_detail as y left join child_basic_detail 
as x on x.district_id =y.area_id left join final_order 
as z on x.child_id =z.child_id where 
(x.district_id='' and z.child_id in(select x.child_id from final_order where dist='' or dist IS NULL))
 or x.child_id in(select z.child_id from final_order where y.area_id COLLATE utf8_unicode_ci 
 in (select division_details.district_id from division_details where 
 division_details.division_id='" . $division_id."') AND STR_TO_DATE(x.idate_of_rescue,'%Y-%m-%d')) 
 AND STR_TO_DATE(x.idate_of_rescue,'%Y-%m-%d') BETWEEN  '".$from."' AND '".$to."' 
 group by area_name order by y.area_name")->result_array();
...
where (x.district_id='' and z.child_id
in(select x.child_id from final_order where dist='' or dist IS NULL)) 
or x.child_id in(select z.child_id from final_order where y.area_id ='".$dist_id."') 
AND STR_TO_DATE(x.idate_of_rescue,'%Y-%m-%d') 
 BETWEEN  '".$from."' AND '".$to."' ")->result_array();
	  ...
	  public function get_education($from,$to,$dist) {
	  	...
	  	if( $roles_id==10 || $roles_id==11 || $roles_id==21 || $roles_id==12 || $roles_id==22)//For LC and LC Operator
	  	{
	  		...
	  		$child_Illiterate = $this->db->query("select sum(education='Illiterate') as num_child,area_id
 from (select y.area_id, y.area_name,x.education from child_area_detail as
y left join child_basic_detail as x on x.district_id =y.area_id and x.education='Illiterate'
left join  final_order as z on x.child_id =z.child_id
 where  x.child_id in(select z.child_id from final_order where y.parent_id ='IND010' and dist='' or dist IS NULL)  AND STR_TO_DATE(x.idate_of_rescue,'%Y-%m-%d') BETWEEN  '".$from."' AND '".$to."')
Z group by area_name order by area_name")->result_array();
	  		
	  		$child_UP = $this->db->query("
select sum(education in('1st','2nd','3rd','4th','5th')) as num_child,
area_id  from (select y.area_id, y.area_name,x.education from child_area_detail 
as y left join child_basic_detail as x on x.district_id =y.area_id 
and x.education in('1st','2nd','3rd','4th','5th') 
left join  final_order as z on x.child_id =z.child_id 
  where  x.child_id in(select z.child_id from final_order where y.parent_id ='IND010' and dist='' or dist IS NULL)  AND STR_TO_DATE(x.idate_of_rescue,'%Y-%m-%d') 
BETWEEN  '".$from."' AND '".$to."') Z group by area_name order by area_name")->result_array();
	  		
          $child_ML = $this->db->query("select sum(education in('6th','7th')) as num_child,
     area_id  from (select y.area_id, y.area_name,x.education from child_area_detail as y left join 
    child_basic_detail as x on x.district_id =y.area_id and x.education in('6th','7th')
left join  final_order as z on x.child_id =z.child_id 
  where  x.child_id in(select z.child_id from final_order where y.parent_id ='IND010' and dist='' or dist IS NULL)  AND STR_TO_DATE(x.idate_of_rescue,'%Y-%m-%d') 
    BETWEEN  '".$from."' AND '".$to."') Z group by area_name order by area_name")->result_array();
	  		
	  		$child_UperP = $this->db->query("
select sum(education in('8th','9th','10th')) as num_child,area_id  
from (select y.area_id, y.area_name,x.education from child_area_detail as y left join 
child_basic_detail as x on x.district_id =y.area_id and x.education in('8th','9th','10th')
left join  final_order as z on x.child_id =z.child_id 
  where x.child_id in(select z.child_id from final_order where y.parent_id ='IND010' and dist='' or dist IS NULL)   AND STR_TO_DATE(x.idate_of_rescue,'%Y-%m-%d') BETWEEN  
'".$from."' AND '".$to."') Z group by area_name order by area_name")->result_array();
	  		
	  		$child_HS = $this->db->query("select sum(education in('11th','12th')) as num_child,area_id 
 from (select y.area_id, y.area_name,x.education from child_area_detail as y left join 
child_basic_detail as x on x.district_id =y.area_id and 
x.education in('11th','12th')  left join  final_order as z on x.child_id =z.child_id 
  where  x.child_id in(select z.child_id from final_order where y.parent_id ='IND010' and dist='' or dist IS NULL)   
AND STR_TO_DATE(x.idate_of_rescue,'%Y-%m-%d') BETWEEN  '".$from."' AND '".$to."') 
Z group by area_name order by area_name")->result_array();
	  		
	  		$child_NS = $this->db->query("select sum(education='' or education='0') as num_child,area_id 
 from (select y.area_id, y.area_name,x.education from child_area_detail as y left join 
child_basic_detail as x on x.district_id =y.area_id and (x.education='' or x.education='0')
 left join  final_order as z on x.child_id =z.child_id 
  where  x.child_id in(select z.child_id from final_order where y.parent_id ='IND010' and dist='' or dist IS NULL)  AND STR_TO_DATE(x.idate_of_rescue,'%Y-%m-%d') BETWEEN 
 '".$from."' AND '".$to."') Z group by area_name order by area_name")->result_array();
	  	...
	  	else if($roles_id==13 || $roles_id==20 )
	  	{
	  		...
	  		$child_Illiterate = $this->db->query("select sum(education='Illiterate') as num_child,
area_id  from (select y.area_id, y.area_name,
x.education from child_area_detail as y left join
 child_basic_detail as x on x.district_id =y.area_id 
and x.education='Illiterate' 
left join  final_order as z on x.child_id =z.child_id
 where  x.child_id in(select z.child_id from final_order where y.parent_id ='IND010' and dist='' or dist IS NULL) and
y.area_id COLLATE utf8_unicode_ci in 
(select division_details.district_id from division_details  
where division_details.division_id='" . $division_id."') 
AND STR_TO_DATE(x.idate_of_rescue,'%Y-%m-%d') 
BETWEEN  '".$from."' AND '".$to."') 
Z group by area_name order by area_name")->result_array();
	  		//$child_UP = $this->db->query("select sum(education in('1st','2nd','3rd','4th','5th')) as num_child,area_id  from (select y.area_id, y.area_name,x.education from child_area_detail as y left join child_basic_detail as x on x.district_id =y.area_id and x.education in('1st','2nd','3rd','4th','5th')  where y.area_id COLLATE utf8_unicode_ci in (select division_details.district_id from division_details  where division_details.division_id='" . $division_id."') AND STR_TO_DATE(x.idate_of_rescue,'%Y-%m-%d') BETWEEN  '".$from."' AND '".$to."') Z group by area_name order by area_name")->result_array();
	  		$child_UP = $this->db->query("select sum(education in('1st','2nd','3rd','4th','5th')) as num_child,
area_id  from (select y.area_id, y.area_name,x.education from child_area_detail as y 
left join child_basic_detail as x on x.district_id =y.area_id 
and x.education in('1st','2nd','3rd','4th','5th')  left join  final_order as z on x.child_id =z.child_id
 where  x.child_id in(select z.child_id from final_order where y.parent_id ='IND010' and dist='' or dist IS NULL) and
y.area_id COLLATE utf8_unicode_ci in 
(select division_details.district_id from division_details  where division_details.division_id='" . $division_id."') 
AND STR_TO_DATE(x.idate_of_rescue,'%Y-%m-%d') BETWEEN  '".$from."' AND '".$to."')
 Z group by area_name order by area_name")->result_array();
	  		...
	  		$child_ML = $this->db->query("select sum(education in('6th','7th')) as num_child,area_id 
 from (select y.area_id, y.area_name,x.education 
from child_area_detail as y left join child_basic_detail as x 
on x.district_id =y.area_id and x.education in('6th','7th') 
left join  final_order as z on x.child_id =z.child_id
 where  x.child_id in(select z.child_id from final_order where y.parent_id ='IND010' and dist='' or dist IS NULL) and y.area_id COLLATE utf8_unicode_ci 
in (select division_details.district_id from division_details  where division_details.division_id='" . $division_id."')
  AND STR_TO_DATE(x.idate_of_rescue,'%Y-%m-%d') BETWEEN  '".$from."' AND '".$to."')
 Z group by area_name order by area_name")->result_array();
	  		...
	  		$child_UperP = $this->db->query("select sum(education in('8th','9th','10th')) as num_child,area_id 
 from (select y.area_id, y.area_name,x.education from 
child_area_detail as y left join child_basic_detail as x on x.district_id =y.area_id 
and x.education in('8th','9th','10th') left join  final_order as z on x.child_id =z.child_id
 where  x.child_id in(select z.child_id from final_order where y.parent_id ='IND010' and dist='' or dist IS NULL) and y.area_id COLLATE utf8_unicode_ci 
in (select division_details.district_id from division_details  
where division_details.division_id='" . $division_id."')
 AND STR_TO_DATE(x.idate_of_rescue,'%Y-%m-%d') BETWEEN  '".$from."' AND '".$to."') Z 
group by area_name order by area_name")->result_array();
	  		...
	  		$child_HS = $this->db->query("select sum(education in('11th','12th')) as num_child,
area_id  from (select y.area_id, y.area_name,x.education from child_area_detail as y
 left join child_basic_detail as x on x.district_id =y.area_id and x.education in('11th','12th') 
 left join  final_order as z on x.child_id =z.child_id
 where  x.child_id in(select z.child_id from final_order where y.parent_id ='IND010' and dist='' or dist IS NULL) and y.area_id COLLATE utf8_unicode_ci
 in (select division_details.district_id from division_details  
where division_details.division_id='" . $division_id."') 
AND STR_TO_DATE(x.idate_of_rescue,'%Y-%m-%d') BETWEEN  '".$from."' AND '".$to."') Z
 group by area_name order by area_name")->result_array();
	  		...
	  		$child_NS = $this->db->query("select sum(education='' or education='0') as num_child,area_id  
from (select y.area_id, y.area_name,x.education from child_area_detail as 
y left join child_basic_detail as x on x.district_id =y.area_id and (x.education='' or x.education='0')
 left join  final_order as z on x.child_id =z.child_id
 where  x.child_id in(select z.child_id from final_order where y.parent_id ='IND010' and dist='' or dist IS NULL) and y.area_id COLLATE utf8_unicode_ci in (select division_details.district_id from division_details  
where division_details.division_id='" . $division_id."') AND STR_TO_DATE(x.idate_of_rescue,'%Y-%m-%d') 
BETWEEN  '".$from."' AND '".$to."') Z group by area_name order by area_name")->result_array();
	  	}
	  	else if(($roles_id==2)||($roles_id==5)||($roles_id==6)||($roles_id==7)||($roles_id==8)||($roles_id==14)||($roles_id==16)||($roles_id==18)||($roles_id==19))//for ls ,leo and all district level
	  	{
...
// 	  		$child_NS = $this->db->query("select sum(education='' or education='0') as num_child,area_id  from (select y.area_id, y.area_name,x.education from child_area_detail as y left join child_basic_detail as x on x.district_id =y.area_id and x.education='' or x.education='0' where y.area_id COLLATE utf8_unicode_ci in (select division_details.district_id from division_details  where division_details.division_id='" . $division_id."') AND STR_TO_DATE(x.idate_of_rescue,'%Y-%m-%d') BETWEEN  '".$from."' AND '".$to."') Z group by area_name order by area_name")->result_array();
	  		...
	  		$child_Illiterate = $this->db->query("select sum(education='Illiterate') as num_child,
area_id  from (select y.area_id, y.area_name,x.education from child_area_detail as 
y left join child_basic_detail as x on x.district_id =y.area_id and x.education='Illiterate'
left join  final_order as z on x.child_id =z.child_id
 where  x.child_id in(select z.child_id from final_order where  dist='' or dist IS NULL) and y.area_id ='".$dist_id."' 
AND STR_TO_DATE(x.idate_of_rescue,'%Y-%m-%d') BETWEEN  '".$from."' AND '".$to."') Z 
group by area_name order by area_name")->result_array();
	  		...
	  		$child_UP = $this->db->query("select sum(education in('1st','2nd','3rd','4th','5th')) as num_child,
area_id  from (select y.area_id, y.area_name,x.education from child_area_detail as y 
left join child_basic_detail as x on x.district_id =y.area_id and x.education in('1st','2nd','3rd','4th','5th') 
 left join  final_order as z on x.child_id =z.child_id
 where  x.child_id in(select z.child_id from final_order where  dist='' or dist IS NULL) and y.area_id ='".$dist_id."' 
 AND STR_TO_DATE(x.idate_of_rescue,'%Y-%m-%d') BETWEEN  '".$from."' AND '".$to."')
 Z group by area_name order by area_name")->result_array();
	  		...
	  		$child_ML = $this->db->query("select sum(education in('6th','7th')) as num_child,area_id 
 from (select y.area_id, y.area_name,x.education from child_area_detail as y
 left join child_basic_detail as x on x.district_id =y.area_id and x.education in('6th','7th')
 left join  final_order as z on x.child_id =z.child_id
 where  x.child_id in(select z.child_id from final_order where  dist='' or dist IS NULL) and y.area_id ='".$dist_id."' AND STR_TO_DATE(x.idate_of_rescue,'%Y-%m-%d')
 BETWEEN  '".$from."' AND '".$to."') Z group by area_name order by area_name")->result_array();
	  		
	  		$child_UperP = $this->db->query("select sum(education in('8th','9th','10th')) as num_child,area_id 
 from (select y.area_id, y.area_name,x.education from child_area_detail as y left join child_basic_detail as x on x.district_id =y.area_id 
and x.education in('8th','9th','10th') left join  final_order as z on x.child_id =z.child_id
 where  x.child_id in(select z.child_id from final_order where  dist='' or dist IS NULL) and y.area_id ='".$dist_id."'  AND 
STR_TO_DATE(x.idate_of_rescue,'%Y-%m-%d')
 BETWEEN  '".$from."' AND '".$to."') Z group by area_name order by area_name")->result_array();
	  		
	  		$child_HS = $this->db->query("select sum(education in('11th','12th')) as num_child,
area_id  from (select y.area_id, y.area_name,x.education 
from child_area_detail as y left join child_basic_detail as x on x.district_id =y.area_id
 and x.education in('11th','12th') 
 left join  final_order as z on x.child_id =z.child_id
 where  x.child_id in(select z.child_id from final_order where  dist='' or dist IS NULL) and y.area_id ='".$dist_id."' 
AND STR_TO_DATE(x.idate_of_rescue,'%Y-%m-%d') BETWEEN  '".$from."' AND '".$to."') Z
 group by area_name order by area_name")->result_array();
	  		...
	  		$child_NS = $this->db->query("select sum(education='' or education='0') as num_child,area_id 
 from (select y.area_id, y.area_name,x.education
 from child_area_detail as y left join child_basic_detail as
 x on x.district_id =y.area_id and (x.education='' or x.education='0')left join  final_order as z on x.child_id =z.child_id
 where  x.child_id in(select z.child_id from final_order where  dist='' or dist IS NULL) and y.area_id ='".$dist_id."'
AND STR_TO_DATE(x.idate_of_rescue,'%Y-%m-%d') BETWEEN  '".$from."' AND '".$to."') Z group by area_name order by area_name")->result_array();
...
where cbd.ls_activate='N' AND STR_TO_DATE(cbd.idate_of_rescue,'%Y-%m-%d') BETWEEN  '".$from."' AND '".$to."' and cbd.district_id='" . $district_id."'  ";
	  ...
	  $query="select child_basic_detail.child_id, child_basic_detail.child_name,child_basic_detail.father_name, 
	   ...
	   child_basic_detail.child_id=final_order.child_id where
