...
//end pabitra
  ...
  if(jqForm[0].ocomplaint_lodge.value=="Yes")
  {
    ...
    if(!jqForm[0].policestation_name.value || jqForm[0].policestation_name.value.length>35)
    {
      ...
      $(".policestation_name_msg").html("This field should be less than  35  characters and please select one option");
     return false;
    }
    else{
...
if(jqForm[0].pincode_out.value)
{
	if(jqForm[0].pincode_out.value.length!=6)
	{
	
  flag=1;
 
  $("#pincode_from_grp").addClass("validate-has-error");
  //$(".contact2").focus();
 ...
 return false;
		  ...
		  $("#pincode_from_grp").removeClass("validate-has-error");
  ...
  /*if (!jqForm[0].iemployer_name.value)
    ...
    return false;
      ...
      if(jqForm[0].complaint_lodge.value=="Yes")
      {
        ...
        if(!jqForm[0].fir_no.value)
        {
          flag=1;
          $("#fir_no_fr_grp").addClass("validate-has-error");
          $(".fir_no").focus();
          $(".fir_no_msg").html("Fir number should not be left blank.");
         return false;
        }else if(jqForm[0].fir_no.value.length>35){
        	flag=1;
            $("#fir_no_fr_grp").addClass("validate-has-error");
            $(".fir_no").focus();
            $(".fir_no_msg").html("Fir number should be less than  35  characters.");
           return false;	
        ...
        if(!jqForm[0].ps_within.value || jqForm[0].ps_within.value.length>35)
        {
          flag=1;
          $("#name_policestation").addClass("validate-has-error");
          $(".ps_within").focus();
          $(".name_policestation_msg").html("Please select one option.");
         return false;
        }
        else{
          $("#name_policestation").removeClass("validate-has-error");
         $(".name_policestation_msg").html("");
        }
