...
var app = angular.module("cpmisInfo",
							...
							if (d.properties && d.properties.NAME1_ == "Bihar") {
								...
								if (d.properties.NAME1_
									...
									var url = "resources/geomaps/"
											...
											//var url = "resources/geomaps/bihar.json";
						...
						scope.mapSetup = function(url, callbackMethod) {
							...
							d3.json(url,function(error, us) {
												...
												if (result) {
													...
													.attr("x",function(d) {
														if (d.properties.NAME1_ == "Jehanabad") {
															return (path.centroid(d)[0])/0.99;
														} else {
															return path.centroid(d)[0];
														}
													})
													.attr("y",function(d) {
														if (d.properties.NAME1_ == "Rohtas") {
															return (path.centroid(d)[1]) / 1.03;// block
														} else if (d.properties.NAME1_ == "Saharsa") {
															return (path.centroid(d)[1]) / 0.98;// block
							...
							function clickHandler(d) {
								clicks++; // count clicks
									scope.selected_areacode=d.properties.utdata.areaCode;
								if (clicks === 1) {
